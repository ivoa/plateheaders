...
was backde, baked or otherwise treated to enhance sensitivity.
...
\cardname{PID} should be used if a plate has a permanent identifier of
some sort, as for instance a Digital Object Identifier (DOI).  Prefer
the HTTP URI form (e.g., \url
{https://doi.org/10.1876/plate/dr.3s/006_0003}) if possible.

While there are no guarantees that any of the artefacts mentioned here
can actually be retrieved, operators can give a URI of a networked
service that at least at one point has served the data in
\cardname{SVC-URI}.  This specification does not say anything about what
URIs these are (obvious candidates include ftp, http, of Virtual
Observatory ivo) or what sort of structure will be found at this URI;
for instance, operators might choose to point to their ObsCore 
\citep{2017ivoa.spec.0509L} service.
